...
export class BeverageSelectionComponent {
  ...
  @Output() orderPlaced = new EventEmitter<Product[]>();  
  ...
  placeOrder(): void {
    if (this.selectedBeverages.length > 0) {
      this.orderPlaced.emit(this.selectedBeverages); 
    } else {
      alert('Please select at least one beverage before placing the order.');
    }
  }
}
